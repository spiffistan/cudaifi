...
inline void reduce_min(int res_index)
{
	...
	if (TY < 8)
		COMPMIN(8 * 32 + res_index);
	__syncthreads(); // reduce to 1 block_row
	if (TY < 4)
		COMPMIN(4 * 32 + res_index);
	__syncthreads(); // reduce to 4 rows
	if (TY < 2)
		COMPMIN(2 * 32 + res_index);
	__syncthreads(); // reduce to 2 rows
	if (TY == 0)
		COMPMIN(32 + res_index); // reduce to 1 row, no need to sync anymore, within 1 warp
	if (TY == 0 && TX < 16)
		COMPMIN(16 + res_index); // reduce to 16 values
	if (TY == 0 && TX < 8)
		COMPMIN(8 + res_index); // reduce to 8 values
	if (TY == 0 && TX < 4)
		COMPMIN(4 + res_index); // reduce to 4 values
	if (TY == 0 && TX < 2)
		COMPMIN(2 + res_index); // reduce to 2 values
	if (TY == 0 && TX == 0)
		COMPMIN(1); // reduce to 1 value
	__syncthreads();
}
...
__global__
void cuda_me_texture(int width, int height, macroblock * mb, uint8_t *prediction)
{
	int left = MX - 16;
	int top = MY - 16;
	...
	{
		prediction[(BY * 8) * width + (BX * 64) + TY * 8 + TX] = ref[(minimum[0].y + TY) * 40 + minimum[0].x + TX];
	}
}
...
extern "C" void c63_motion_estimate(struct c63_common *cm, struct cuda_frame *cframe)
{
	...
	cuda_me_texture<<<cframe->me_blockDim_Y, cframe->me_threadDim>>>(cm->ypw, cm->yph, cframe->mbs[0], cframe->predicted->Y);
	...
	cuda_me_texture<<<cframe->me_blockDim_UV, cframe->me_threadDim>>>(cm->upw, cm->uph, cframe->mbs[1], cframe->predicted->U);
...
cuda_me_texture<<<cframe->me_blockDim_UV, cframe->me_threadDim>>>(cm->vpw, cm->vph, cframe->mbs[2], cframe->predicted->V);
}
