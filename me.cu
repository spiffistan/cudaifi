...
extern "C" void c63_motion_estimate(struct c63_common *cm, struct cuda_frame *cframe) {
	...
	cuda_me_texture<<<cframe->me_blockDim_Y, cframe->me_threadDim,0,cframe->stream>>>(cm->ypw, cm->yph, cframe->mbs[0], cframe->predicted->Y);
	...
	cuda_me_texture<<<cframe->me_blockDim_UV, cframe->me_threadDim,0,cframe->stream>>>(cm->upw, cm->uph, cframe->mbs[1], cframe->predicted->U);
	...
	cuda_me_texture<<<cframe->me_blockDim_UV, cframe->me_threadDim,0,cframe->stream>>>(cm->vpw, cm->vph, cframe->mbs[2], cframe->predicted->V);
}
