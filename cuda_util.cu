...
void cuda_store_residuals(struct c63_common *cm, workitem_t *work)
{
	...
	cudaMemcpyAsync(work->residuals->Vdct, cframe->residuals->Vdct, cm->vpw * cm->vph * sizeof(int16_t), cudaMemcpyDeviceToHost, cframe->stream);
...
extern "C" void cuda_run(struct c63_common *cm, workitem_t *work)
{
	...
	cudaStreamSynchronize(cframe->stream);
}
