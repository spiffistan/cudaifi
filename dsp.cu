...
inline void reduce_min(int res_index) {
	//reduce to 2 block rows
	minimum[res_index] = min_swap(minimum[res_index], minimum[16 * 32 + res_index]);

	//reduce to 1 block_rows
	if (threadIdx.y < 8) {
		minimum[res_index] = min_swap(minimum[res_index], minimum[8 * 32 + res_index]);
	}
	__syncthreads();

	//reduce to 4 rows
	if (threadIdx.y < 4) {
		minimum[res_index] = min_swap(minimum[res_index], minimum[4 * 32 + res_index]);
	}
	__syncthreads();

	//reduce to 2 rows
	if (threadIdx.y < 2) {
		minimum[res_index] = min_swap(minimum[res_index], minimum[2 * 32 + res_index]);
	}
	__syncthreads();

	//reduce to 1 row
	if (threadIdx.y == 0) {
		minimum[res_index] = min_swap(minimum[res_index], minimum[32 + res_index]);
	}
	__syncthreads();

	//reduce to 16 values
	if (threadIdx.y == 0 && threadIdx.x < 16) {
		minimum[res_index] = min_swap(minimum[res_index], minimum[16 + res_index]);
	}
	__syncthreads();

	//8
	if (threadIdx.y == 0 && threadIdx.x < 8) {
		minimum[res_index] = min_swap(minimum[res_index], minimum[8 + res_index]);
	}
	__syncthreads();

	//4
	if (threadIdx.y == 0 && threadIdx.x < 4) {
		minimum[res_index] = min_swap(minimum[res_index], minimum[4 + res_index]);
	}
	__syncthreads();

	//2
	if (threadIdx.y == 0 && threadIdx.x < 2) {
		minimum[res_index] = min_swap(minimum[res_index], minimum[2 + res_index]);
	}
	__syncthreads();

	//1
	if (threadIdx.y == 0 && threadIdx.x == 0) {
		minimum[res_index] = min_swap(minimum[res_index], minimum[1]);
	}
	__syncthreads();
}

__global__ void cuda_me_texture(int width, int height, macroblock * mb) {
